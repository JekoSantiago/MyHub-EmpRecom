...
//-- NPA ACCEPTANCE --//
    var tbl_npa_accept = $('#tbl_npa_accept').DataTable({
        processing: true,
        serverSide: true,
        scrollX: true,
        ordering: false,
        ajax      : {
            url: WebURL + '/npa-accepptanceDT',
            method: 'POST',
            data: function (data) {
        ...
        columns   :[
            ...
            {render:function(data, type, row){
                var view = '';
                    view = '<a href="javascript:void(0)" class="text-info viewNPA" > <i class="mdi mdi-24px mdi-eye"></i></a>' ;
                return view;
            }}
        ],
        language: {
    ...
    $('body').on('click','.viewNPA',function(e){
        var data = tbl_npa_accept.row( $(this).parents('tr') ).data();
        var EmpID = data.Employee_ID

        $('#modal_npa').modal('show');
        var remoteLink = WebURL + '/accept-showNPA/'+EmpID;

        $("#modal_npa").find('.modal-body').html('<div class="text-center"><div class="spinner spinner-border"></div></div>');
        $('#modal_npa').find('.modal-body').load(remoteLink,function () {
        });

    })

    $('body').on('click','#btnAcceptNPA',function(){
        var empID = $('#EmpID').val();
        var empNo = $('#EmpNo').val();
        swal.fire({
            title: 'Accept',
            ...
            preConfirm:(done) =>
            {
                ...
                var PIN = Swal.getPopup().querySelector('#pin').value
                    $.post(WebURL + '/emppin-check',{PIN:PIN,empID:empID,empNo:empNo},function(data){
                        if(data == 1)
                        {
                            $.post(WebURL + '/accept-npa',{empID:empID},function(data){
                                if(data.num>=0)
                                {
                                    swal.fire({
                                        title: "Success!",
                                        ...
                                        $('#modal_npa').modal('hide');
                                        tbl_npa_accept.ajax.reload(null,false);
                                    });
                                }
                                else
                                {
